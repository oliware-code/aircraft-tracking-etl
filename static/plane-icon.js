function planeIcon(onGround, heading, label, epoch, route, trackedBy) {
  ...
  // watchlists are visually distinguishable on the map at a glance. Nearby-traffic
  // markers (untracked aircraft near a watched_airports entry) get a muted slate
  // color instead, so they read as background context, not a third watchlist.
  const isNearby = trackedBy === 'nearby';
  const airborneColor = trackedBy === 'callsign' ? '#fb923c' : isNearby ? '#94a3b8' : '#38bdf8';
  const airborneStroke = trackedBy === 'callsign' ? '#7c2d12' : isNearby ? '#334155' : '#0c4a6e';
  const color = onGround ? '#f8fafc' : airborneColor;
  const stroke = onGround ? '#0f172a' : airborneStroke;
  const size = isNearby ? 20 : 30;
  const opacity = isNearby ? 0.75 : 1;
